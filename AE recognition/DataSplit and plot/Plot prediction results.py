...
figure_path = '/media/lj/MachineLearning/AE recognition/Data/HTL data/11th/td/plot/'
os.makedirs(figure_path, exist_ok=True)
defect_file_path_1 = '/media/lj/MachineLearning/AE recognition/Data/HTL data/11th/td/'
...
detection_result = pd.read_csv('/media/lj/MachineLearning/AE recognition/Data/HTL data/11th/processed/results/1.csv')
...
for filename in filename_list:
    ...
    if data_type == 'Y':
        ...
        split_file = pd.read_csv(os.path.join(defect_file_path_1, filename))
        ...
        # plt.legend([print_label], loc='upper left')
    ...
    # if data_type == 'X':
    #     if prediction_result == 0:
    #         print_label = 'True Negative'
    #     if prediction_result == 1:
    #         print_label = 'False Positive'
    #     split_file = pd.read_csv(os.path.join(noise_file_path, filename))
    #     acoustic_data = split_file.iloc[:, 1]
    #     acoustic_data = acoustic_data.to_numpy()
    #     xpt_start = ' '.join(filename.split(' ')[1:7])
    #     xpt_end = filename.split('-')[1]
    #     pt_step = 1 / sampling_rate
    #     fig_title = 'Noise' + xpt_start
    #
    #     # time_sequence = np.arange(float(xpt_start), float(xpt_end), pt_step)
    #     time_sequence = np.arange(0, 0.11, pt_step)
    #     defect_start = 0.05
    ...
    #     plt.figure(2)
    #     start_time = filename.split(' ')[1]
    #     end_time = filename.split(' ')[4]
    #     title = 'Noise ' + start_time + ' sec to ' + end_time + ' sec'
    #     melspec_total = librosa.feature.melspectrogram(acoustic_data, n_mels=512)
    #     logspec_total = librosa.amplitude_to_db(melspec_total)
    #     librosa.display.specshow(logspec_total, cmap='jet', sr=sr, x_axis='time', y_axis='linear')
    #     plt.colorbar(format='%+2.0f dB')
