...
sr = 1000000
figure_path = '/media/lj/MachineLearning/AE recognition/Data/Pencil lead/Pencil lead break/extract/N/processed/mel/fig/'
...
def extract_features(parent_dir, sub_dir, file_ext="*.csv",
    ...
    for fn in glob.glob(os.path.join(parent_dir, sub_dir, file_ext)):
        ...
        # _, _, sgram = signal.spectrogram(sound_clip, sr, nperseg=256)
...
kf = KFold(n_splits=5)
for train_index, val_index in kf.split(folds):
    x_train, y_train = [], []
    x_test, y_test = [], []
    ...
    validation_data = np.load("{0}/{1}.npz".format(load_dir, folds[val_index][0]), allow_pickle=True)
    x_val = validation_data["features"]
    y_val = validation_data["labels"]

    y_validation = np.reshape(y_val, (-1, len(y_val)))
    ...
    model.save('/media/lj/MachineLearning/AE recognition/Data/Pencil lead/Pencil lead break/extract/N/processed/model/')
