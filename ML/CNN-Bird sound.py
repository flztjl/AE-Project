...
def extract_features(parent_dir, sub_dir, file_ext="*.wav",
    ...
    features, labels= [], []
    label_data = pd.read_csv('/media/lj/AE Project/Dropbox/Project data/ff1010bird_metadata_2018.csv')

    for fn in glob.glob(os.path.join(parent_dir, sub_dir, file_ext)):
        segment_log_specgrams, segment_labels = [], []
        sound_clip, sr = librosa.load(fn)
        file_name = fn.split('/')[8].split('.')[0]
        tag = label_data[label_data.itemid == int(file_name)].index.tolist()[0]
        label = label_data.iloc[tag, 2]
...
parent_dir = '/media/lj/AE Project/Dropbox/Project data/wav/'
save_dir = '/media/lj/AE Project/Dropbox/Project data/wav/processed/'
os.makedirs(save_dir, exist_ok=True)

folds = sub_dirs = np.array(['train', 'test', 'validate'])
...
load_dir = '/media/lj/AE Project/Dropbox/Project data/wav/processed/'
# kf = KFold(n_splits=5)
...
checkpoint_dir = "/media/lj/AE Project/Dropbox/Project data/wav/"
...
cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_dir,
                                                 ...
                                                 verbose=1)
history = model.fit(x_train, y_train, validation_data=(x_validation, y_validation), epochs=40, batch_size=24,
...
# y_compose = list(zip(y_true, y_pred))
# y_compose_df = pd.DataFrame(y_compose)
# y_compose_df.to_csv('/media/lj/MachineLearning/AE recognition/Data/HTL data/processed/7th-1st.csv')
